...
def generate_random_comparison_positions(p_length):
    example = [i for i in range(p_length)]
    sample_num = len(merge_sort(example))
    all_digits = [i for i in range(int(p_length * (p_length-1) / 2))]
    sample_digits = np.random.choice(all_digits, size=sample_num, replace=False)
    # print(sample_digits)
    return sample_digits


def featurize(x):
    sample_digits = generate_random_comparison_positions(x.size(1))
    featurized_x = []
...
def bo_loop(fname, kernel_type, start=0, end=20):
    n_init = 20
    n_evals = 200
    for nruns in range(start, end):
        ...
        for num_iters in range(n_init, n_evals):
            ...
            if kernel_type == 'random_selection':
            ...
            file_name = 'pl_botorch_'+kernel_type+'_EI_30'
...
if __name__ == '__main__':
    ...
    parser_.add_argument('--kernel_type', dest='kernel_type', type=str, default='random_selection')
    parser_.add_argument('--start', dest='start', type=int, default=0)
    parser_.add_argument('--end', dest='end', type=int, default=20)
    args_ = parser_.parse_args()
    kwag_ = vars(args_)
    bo_loop(kwag_['fname'], kwag_['kernel_type'], kwag_['start'], kwag_['end'])
