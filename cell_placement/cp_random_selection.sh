...
j=$((i-1))
WORKDIR="cp_random_selection_${i}"

mkdir -p $WORKDIR

cp cp_simulator cp_random_selection.py bo_pl.mat ex_10_40_2_10.dat output_pl.txt permutation.txt run_sdp_pl.m $WORKDIR/
cd $WORKDIR/
chmod +x cp_simulator

python cp_random_selection.py --start $j --end $i
