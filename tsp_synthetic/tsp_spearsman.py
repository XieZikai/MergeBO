...
def featurize(x):
    ...
    return torch.tensor(featurized_x/normalizer)
...
def bo_loop(dim, benchmark_index, kernel_type):
    ...
    for nruns in range(20):
        ...
        for num_iters in range(n_init, n_evals):
            ...
            torch.save({'inputs_selected':train_x, 'outputs':outputs, 'train_y':train_y}, 'tsp_botorch_spearsman_EI_dim_'+str(dim)+'benchmark_index_'+str(benchmark_index)+'_nrun_'+str(nruns)+'.pkl')
