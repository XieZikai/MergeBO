...
j=$((i-1))
WORKDIR="floorplan_random_selection_${i}"

mkdir -p $WORKDIR

cp floorplan_simulation floorplan_random_selection.py bo_fp.mat b1_floorplan.blk b2_floorplan.blk init.plt output_floorplan.txt pcb_dim_10_1.mat permutation.txt run_sdp_fp.m $WORKDIR/
cd $WORKDIR/
chmod +x floorplan_simulation

python floorplan_random_selection.py --start $j --end $i
